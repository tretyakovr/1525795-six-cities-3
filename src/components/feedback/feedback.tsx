...
function Feedback({offerId}: FeedbackProps): JSX.Element {
  const [rating, setRating] = useState<number>(0);
  const commentText: React.LegacyRef<HTMLTextAreaElement> | null = useRef<HTMLTextAreaElement>(null);
  const refForm: React.LegacyRef<HTMLFormElement> | null = useRef<HTMLFormElement>(null);
  ...
  const handleSubmit = (evt: React.FormEvent<HTMLFormElement> | undefined) => {
    ...
    const comment: string | undefined = String(commentText.current?.value);
    dispatch(sendCommentAction({offerId: offerId, comment: comment, rating: rating}));

    refForm.current?.reset();
  };

  return (
    <form ref={refForm} className="reviews__form form" action="#" method="post" onSubmit={handleSubmit}>
